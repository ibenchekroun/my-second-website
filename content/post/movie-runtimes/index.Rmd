...
To learn how Blockbusters, Mid-Budget, and the 'Before' and 'Now' time periods were defined, find out [here](#settingparameters).
...
get_scatterplot_years <- function(the_data, func){
    the_data$Runtime <- the_data$Runtime_numeric
    the_data$Year <- the_data$Year_numeric
    
    p <- the_data %>% 
      ggplot(aes(x = Year, y = Runtime, label = Title)) +
      geom_point() +
...
filtered_data6 <- filter_data_ba_fn(before_year1,
    ...
    l1 <- get_scatterplot_years(filtered_data6, "umm")
    ...
    # 
    t1 <- l1[[3]]
    t1 <- coalesce(ifelse(t1 > 0, t1, 0), 0)
    # t2 <- l2[[3]]
    # t3 <- l3[[3]]
    # t4 <- l4[[3]]
    ...
    # print(max)
    min_max_2 <- list(list(p1), c(min, max) #, 
    )
    p <- min_max_2[[1]][[1]]
    r <- min_max_2[[2]]
    # print("START R")
    ...
    # t <- min_max_pc_2()[[3]]
    if(filtered_data6 %>% nrow() >= 1){
    gg <- ggplotly(p + 
                     ggtitle("Blockbuster movies of 2009-2022 (2020 excluded)") + lims(x = c(2008, 2022+1), y = r) + scale_size_continuous(limits=c(1,110)), tooltip = c("x", "y", "label", "label2", "custom_variable" = ~"Runtime")
    )}else{gg <- NoDataPlotly()}
    
    for (i in 1:length(gg$x$data)){
      if (!is.null(gg$x$data[[i]]$name)){
        gg$x$data[[i]]$name =  gsub("^\\(","",str_split(gg$x$data[[i]]$name,",")[[1]][1])
      }
    }
    
```

```{r, echo = FALSE, fig.cap='Runtime of the top 50 budget films of each year from 2009 to 2022, excluding 2020'}
...
```{r, echo = FALSE}
dd <- filtered_data6 # print(colnames(filtered_data4()))

    filtered_data_blockbusters <- dd %>% 
      mutate(Budget_2020_usd_numeric = round(Budget_2020_usd_numeric),
             img_url = paste0("<a href = '", url, "' <figure> <style> { color: darkblue; } /* CSS link color */
  </style>","<img src = '", img_url, "' height=120 width=100</img> ", "<figcaption style=\"text-align-center\"><strong>", Title, "</strong> (", Year, ")", "</figcaption>
</figure>")) %>% # filter_by_genres(filtered_data2, genres_from_plot_number_g()) %>%
      select(img_url,Runtime_numeric, Budget, Genres, Budget_2020_usd_numeric) %>%
      arrange(-Budget_2020_usd_numeric) %>% 
      rename(`Runtime (mins)` = Runtime_numeric, Movie = img_url) %>% select(-Budget_2020_usd_numeric)
#     filtered_data$img_url <- with(filtered_data, paste0("<figure>", "<style>
#     a { color: darkblue; } /* CSS link color */
#   </style>", img_url, "height=120></img>", "height=120 width=100></img>"), "<figcaption style=\"text-align-center\"><strong>", Title, "</strong> (", Year, ")", "</figcaption>
# </figure>")))
    # filtered_data$img_url = <img src="dinosaur.jpg" alt="Dinosaur" />
    # filtered_data$img_url[1] = '<img src=\"https://www.basketball-reference.com/req/202106291/images/players/hartjo01.jpg\" height=150></img>'
    # print(filtered_data$Movie[1])
    # print(dd[2260,]$url)
    # print(dd %>% filter(is.na(Title)))
...
filtered_data6 <- filter_data_ba_fn(before_year1,
    ...
    if(filtered_data6 %>% nrow() >= 1){
    gg <- ggplotly(p + 
                     ggtitle("Mid-Budget movies of 2009-2022 (2020 excluded)") + lims(x = c(2008, 2022+1), y = r) + scale_size_continuous(limits=c(1,110)), tooltip = c("x", "y", "label", "label2", "custom_variable" = ~"Runtime")
    )}else{gg <- NoDataPlotly()}
...
```{r, echo = FALSE, fig.cap='Runtime of the 51st to 100th budget films of each year from 2009 to 2022, excluding 2020'}
...
Gross, budget, and genres are properly defined in the [glossary](#Glossary). 
...
```{r, include = FALSE}
movie_trait_definitions_df <- data.frame(
  movie_trait_readable = c("Budget", 
                                                      "Gross",
                                                      "Action", 
                                                      "Non-Action Comedy" 
                                                      ),
  definition = c("The budget listed on IMDb.", 
                                                                       "The US & Canada gross revenue listed on IMDb.",  
                                                                       "One of the genres listed on IMDb is Action.",
                                                                       "One of the genres listed on IMDb is Comedy AND none of the genres listed is Action."))
...
kable(movie_trait_definitions_df %>% select(movie_trait_readable, definition) %>% rename("Movie Trait" = movie_trait_readable, "Definition" = definition), format = "html", table.attr = "style='width:60%;'", caption = "Movie Trait Definitions") %>% 
  kableExtra::kable_styling()
```

### A2: List of the Blockbuster and Mid-Budget movies {#listOfMovies}

### Blockbuster movies
```{r, echo = FALSE}
 DT::datatable(
      filtered_data_blockbusters,
      caption = "Interactive Table 1: The top 50 budget films of each year from 2009 to 2022, excluding 2020",
      # container = sketch, 
      rownames = FALSE, 
      escape = FALSE, 
      options = list(
...
```{r, echo=FALSE}
    DT::datatable(
      filtered_data_midbudget,
      caption = "Interactive Table 2: The 51st to 100th budget films of each year from 2009 to 2022, excluding 2020",
      # container = sketch, 
      rownames = FALSE, 
      escape = FALSE, 
      options = list(
...
### A3: More Limitations {#fulllimitations}
